...
\end{quote}
\begin{quote}
    \textbf{Ma trận Transform (Affine):} Thuật toán chuyển đổi chỉ mục phần tử (cột, hàng) của mảng bộ nhớ (NumPy) thành tọa độ phẳng (X, Y) trên không gian bản đồ (thường tính bằng mét).

    \medskip
    \textbf{Hệ tọa độ tham chiếu (CRS):} Hệ thống định nghĩa mặt phẳng bản đồ, đồng thời chứa các phương trình toán học để biến đổi qua lại giữa tọa độ phẳng (X, Y) và tọa độ địa lý mặt cầu (Kinh độ, Vĩ độ).
\end{quote}
